...
void Game::remove_hook(GameHooks *gh)
{
...
inline void Game::wipe_info()
{
	_bad_free_count = 1;
	_possible_moves = -1;
}

void Game::check_level_blockage(int r, int c, int l) const
{
	Tile *t = grid(r, c, l);
	if (t->real()) {
		r = t->row();
...
uint32_t Game::seed_to_board_number(uint32_t seed, bool solvable)
{
	assert((seed & 0xC0000000U) == 0);
	return ((seed & 0x3FFF0000) << 1) | (solvable ? 0x10000 : 0) | (seed & 0xFFFF);
}

void Game::board_number_to_seed(uint32_t board_number, uint32_t &seed, bool &solvable)
{
	assert((board_number & 0x80000000U) == 0);
	solvable = (board_number & 0x10000) != 0;
	seed = ((board_number & 0x7FFE0000) >> 1) | (board_number & 0xFFFF);
}

void Game::assign(uint32_t seed)
{
	...
	for (int i = 0; i < ntiles; i++) {
		_tiles[i]->assign(permuted_tiles[i], _tileset->match(permuted_tiles[i]));
	}

	delete[] permuted_tiles;
}

void Game::assign_solvable(uint32_t seed)
{
	seed &= 0x3FFFFFFFU;
...
void Game::start(uint32_t the_seed, bool solvable)
{
	_taken = 0;
	_left = _tiles.size();
	_solution.clear();
...
void Game::start_specific(uint32_t board_number)
{
...
void Game::move(Tile *t1, Tile *t2, bool was_user)
{
...
void Game::count_possible_moves()
{
	int nmatch = nmatches();
	if (_bad_free_count) {
		make_free_count();
	...
	for (i = 0; i < nmatch; i++) {
		if (_free_count[i] == 2) {
			j += 1;
		} else if (_free_count[i] == 3) {
			j += 3;
		} else if (_free_count[i] == 4) {
			j += 6;
		}
	}
...
static int tile_sorter(const void *v1, const void *v2)
{
	Tile *t1 = *(Tile**) v1;
	Tile *t2 = *(Tile**) v2;
	if (t1->row() != t2->row()) {
...
bool Game::place_tile(int r, int c, int l)
{
	if (r > 1 && c > 1 && l >= 0 && r < TILE_ROWS - 3 && c < TILE_COLS - 3 && l < TILE_LEVS - 1) {
...
bool Game::layout_kyodai_file(FILE *f)
{
	// skip `Kyodai 3.0'
	int c = getc(f);
	while (c != '\n' && c != EOF) {
...
int Game::layout_file(const char *filename)
...
int Game::left_count(int i) const
{
	assert(i >= 0 && i < nmatches());
